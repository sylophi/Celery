...
import type { FolderState, Group, ModFile, Section as ModSection } from "@shared/schemas";
...
export function DetailPanel({
  ...
  const optionalDeps = [...(index.optionalDeps.get(file.fileName) ?? [])].toSorted();
  const dependents = [...(index.dependents.get(file.fileName) ?? [])].toSorted();
  const optionalDependents = [...(index.optionalDependents.get(file.fileName) ?? [])].toSorted();
  ...
  const defaultSection: ModSection = dependents.length > 0 ? "dependency" : "mod";
  const effectiveSection: ModSection = dependencySet.has(file.fileName) ? "dependency" : "mod";
  const setSection = (section: ModSection) => {
    const overrides = { ...folderState.sectionOverrides };
    if (section === defaultSection) delete overrides[file.fileName];
    else overrides[file.fileName] = section;
    saveState.mutate({ ...folderState, sectionOverrides: overrides });
  };
  ...
  return (
    <div className="shadow-floating absolute inset-y-3 right-3 z-40 flex w-72 flex-col overflow-hidden rounded-xl border border-border bg-popover text-popover-foreground">
      <div className="shrink-0 border-b border-border p-3">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            <div className="truncate text-sm font-medium">{displayName(file.fileName)}</div>
        ...
        <div className="mt-2.5 flex items-center gap-1.5">
          ...
          >
            <StarIcon className={cn("size-3.5", file.favorite && "fill-current")} />
          </Button>
          <Button
            variant="outline"
            size="icon-sm"
            ...
            onClick={() => void window.api.shell.showItemInFolder(`${folder}/${file.fileName}`)}
          >
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-3">
        {file.parseError && (
          <p className="mb-3 text-xs text-destructive">manifest problem: {file.parseError}</p>
        )}
        {missing.length > 0 && (
        ...
        <Section label={`needs · ${hardDeps.length}`} empty="needs nothing beyond Everest">
          {hardDeps.map((dep) => (
            <DepRow key={dep} fileName={dep} index={index} onSelect={onSelect} />
          ))}
        </Section>
        ...
        <Section label={`needed by · ${dependents.length}`} empty="nothing depends on this">
          {dependents.map((dep) => (
            <DepRow key={dep} fileName={dep} index={index} onSelect={onSelect} />
          ))}
        </Section>
        {optionalDependents.length > 0 && (
          <Section label={`optionally needed by · ${optionalDependents.length}`}>
        ...
        <div className="mb-3">
          ...
          <SegmentedControl<ModSection>
            options={[
              { value: "mod", label: "mod", selected: effectiveSection === "mod" },
              {
                value: "dependency",
                label: "dependency",
        ...
        {file.entries.length > 1 && (
          <Section label={`declares ${file.entries.length} mods`}>
            {file.entries.map((entry) => (
              <li key={entry.name} className="flex items-baseline gap-1.5 px-1.5 py-1 text-xs">
...
function DepRow({
  ...
  return (
    <li className="group relative flex h-7 items-center gap-1.5 rounded-md px-1.5 transition-colors hover:bg-muted">
      ...
      />
      <span className="min-w-0 flex-1 truncate text-xs">{displayName(fileName)}</span>
      <span className="tabular shrink-0 text-[10px] text-muted-foreground/60">
